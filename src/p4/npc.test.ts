...
import { npcState, npcTarget, seatHome, centerSafeNear, gazeFormationSpot, finalMemoryParams } from "@/p4/npc";
...
describe("中央 AoE 回避：グランドクロス / サンダガ / ブリザガ", () => {
  ...
  it("マジックアウト t=87 で全席が最終記憶 AoE(cross) を回避", () => {
    const fm = finalMemoryParams(setup);
    for (const seat of ALL_SEATS) {
      const { pos } = npcState(setup, seat, 87);
      expect(centerAoeSafeGeometry(pos, fm, "cross"), `seat ${seat} not magic-out safe`).toBe(true);
    }
  });
