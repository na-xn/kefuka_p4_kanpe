...
describe("evaluate(role): ロール別水雷判定（PlayArena 採点パス回帰）", () => {
  ...
  // 早(エクスデス北)フレームで DPS が「固定ウェイマーク(標準 ZONES カーディナル)」に
  // 立ったときの合否は、その固定点が回転後 DPS カーディナルと一致するときだけ true。
  // = 「TH の場所(固定A)でも成功」はロール盲のバグではなく回転の偶然である、を固定する。
  it("早/exdeath: DPS は回転後 DPS カーディナルでのみ合格・回転後 TH では常に不合格（固定点合格は回転の偶然）", () => {
    const stillDir = { x: 0, y: -1 };
    for (let angle = 0; angle < 8; angle++) {
      const ez = exdeathZones(angle);
      for (const isStack of [true, false]) {
        const req: RequiredAction = { kind: isStack ? "filler" : "spread", label: "" };
        const dpsPt = roleCardinalPoint("DPS", isStack, ez);
        const thPt = roleCardinalPoint("TH", isStack, ez);
        // 回転後 DPS カーディナル中心 → 合格。
        expect(evaluate(req, dpsPt, stillDir, false, undefined, undefined, ez, "DPS").ok).toBe(true);
        // 回転後 TH カーディナル中心 → 常に不合格（ロール別判定が効いている）。
        expect(evaluate(req, thPt, stillDir, false, undefined, undefined, ez, "DPS").ok).toBe(false);
        // 固定ウェイマーク(標準 ZONES)に立ったときの合否は、その固定点が回転後 DPS
        // カーディナルとほぼ一致するか（< ZONE_RADIUS）に等しい（回転の偶然）。
        for (const z of ["h12", "h3", "h6", "h9"] as const) {
          const okAtFixed = evaluate(req, ZONES[z], stillDir, false, undefined, undefined, ez, "DPS").ok;
          const nearRotDps = Math.hypot(ZONES[z].x - dpsPt.x, ZONES[z].y - dpsPt.y) < ZONE_RADIUS;
          expect(okAtFixed).toBe(nearRotDps);
        }
      }
    }
  });

  it("roleCardinalLabel: TH 頭割り=A(北)/散開=D(西)・DPS 頭割り=C(南)/散開=B(東)", () => {
    expect(roleCardinalLabel("TH", true)).toBe("A(北)");
    expect(roleCardinalLabel("TH", false)).toBe("D(西)");
    expect(roleCardinalLabel("DPS", true)).toBe("C(南)");
    expect(roleCardinalLabel("DPS", false)).toBe("B(東)");
  });
});
