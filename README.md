...
🌐 **web 版（ブラウザ / スマホ）**: <https://kefuka-p4-kanpe.na-xn.app/> — インストール不要。スマホでセカンド画面として使えます。ただし**オーバーレイ常駐・フォーカス非奪取・グローバルキー入力はデスクトップ版のみ**（web では非表示）。読み上げはブラウザでも動作します。
...
- GC 担当は「**雷 / 水 / 加速度**」3択＋「**早 / 遅**」を分離。つなみ/ほのおの早遅は種類から自動（炎=早・つなみ=遅）。担当・呪詛・2回目の種類などの**排他関係を自動補助**して入力タップを最小化
- 自分が担当する**加速度爆弾・呪詛発生源**のステップを★強調
- **練習モード**（右クリック→モード「練習」）: ランダム生成のお題で判定・立ち位置を練習。**ソロ**（カンペ練習＝自動/手動入力＋答え合わせ／操作プレイ＝アリーナで自機を動かしギミック回避）と、**最大8人セッション**（Cloudflare Durable Objects で同期・ロール別ロビー Tank2/Healer2/DPS4・空席は NPC 自動プレイ・操作プレイは全員の位置をリアルタイム共有）。詳細は [`release/README.md`](release/README.md) の「練習モード」
...
<https://kefuka-p4-kanpe.na-xn.app/> をブラウザで開くだけ（インストール不要）。スマホでも動作し、ヘッダーの ⚙ から設定を開けます。

- 同じ判定入力・処理タイムライン・音声読み上げが使えます。
- **デスクトップ専用機能（オーバーレイ常駐・フォーカス非奪取・F1〜F3 グローバルキー入力・閉じるボタン）は web では非表示**です。
- 静的SPAなので Cloudflare Workers（Static Assets）でホスティング（下記「web デプロイ」）。
